...
func (lq *LikeQuery) QueryFeedback() *FeedbackQuery {
	query := &FeedbackQuery{config: lq.config}
...
func (lq *LikeQuery) FirstID(ctx context.Context) (id int64, err error) {
	var ids []int64
...
func (lq *LikeQuery) FirstIDX(ctx context.Context) int64 {
...
// Returns a *NotSingularError when exactly one Like entity is not found.
...
func (lq *LikeQuery) OnlyID(ctx context.Context) (id int64, err error) {
	var ids []int64
...
func (lq *LikeQuery) OnlyIDX(ctx context.Context) int64 {
...
func (lq *LikeQuery) IDs(ctx context.Context) ([]int64, error) {
	var ids []int64
...
func (lq *LikeQuery) IDsX(ctx context.Context) []int64 {
	ids, err := lq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (lq *LikeQuery) Clone() *LikeQuery {
	...
	return &LikeQuery{
		...
		withFeedback: lq.withFeedback.Clone(),
...
func (lq *LikeQuery) WithFeedback(opts ...func(*FeedbackQuery)) *LikeQuery {
	query := &FeedbackQuery{config: lq.config}
	for _, opt := range opts {
		opt(query)
	}
	lq.withFeedback = query
	return lq
}
...
func (lq *LikeQuery) sqlAll(ctx context.Context) ([]*Like, error) {
	...
	if query := lq.withFeedback; query != nil {
		ids := make([]int64, 0, len(nodes))
		nodeids := make(map[int64][]*Like)
		for i := range nodes {
			if nodes[i].feedback_like == nil {
				continue
			}
			fk := *nodes[i].feedback_like
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "feedback_like" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Feedback = n
			}
		}
	}
